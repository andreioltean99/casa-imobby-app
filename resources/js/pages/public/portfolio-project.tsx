...
function ExternalListingPortals({
    ...
    return (
        <div className="mb-6 flex flex-col gap-3 rounded-xl border border-border/80 bg-card/70 px-4 py-4 shadow-sm ring-1 ring-black/[0.04] sm:flex-row sm:items-center sm:justify-between sm:gap-5 dark:bg-card/40 dark:ring-white/[0.05]">
            <p className="shrink-0 text-xs font-semibold tracking-[0.14em] text-muted-foreground uppercase sm:min-w-[8rem]">
            ...
            <div className="flex flex-wrap items-center gap-8 sm:justify-end sm:gap-10 lg:gap-12">
                {storiaUrl ? (
                    <a
                        ...
                        className="inline-flex shrink-0 rounded-md ring-offset-2 transition-opacity hover:opacity-90 focus-visible:ring-2 focus-visible:ring-brand focus-visible:outline-none"
                        aria-label={
                            tPortfolio.external_storia_aria ??
                            'Open this listing on Storia'
                        }
                    >
                        <img
                            src={STORIA_LOGO_SRC}
                ...
                {imobiliareUrl ? (
                    <a
                        ...
                        className="inline-flex shrink-0 rounded-md ring-offset-2 transition-opacity hover:opacity-90 focus-visible:ring-2 focus-visible:ring-brand focus-visible:outline-none"
                        aria-label={
                            tPortfolio.external_imobiliare_aria ??
                            'Open this listing on Imobiliare.ro'
                        }
                    >
                        <img
                ...
                {olxUrl ? (
                    <a
                        ...
                        className="inline-flex shrink-0 rounded-md ring-offset-2 transition-opacity hover:opacity-90 focus-visible:ring-2 focus-visible:ring-brand focus-visible:outline-none"
                        aria-label={
                            tPortfolio.external_olx_aria ??
                            'Open this listing on OLX'
                        }
                    >
                        <img
                            src={OLX_LOGO_SRC}
...
export default function PortfolioProjectPage({
    ...
    const tPortfolio =
        (translations?.portfolio as Record<string, string> | undefined) ?? {};
    const tUnits =
        (translations?.units as Record<string, string> | undefined) ?? {};
    ...
    const c = contact ?? {};
    const emailDisplay = (
        (c.email as string | undefined) ?? 'office@casa-imobby.ro'
    )
        .toString()
        .trim();
    const phoneRaw = ((c.phone as string | undefined) ?? '').toString().trim();
    const telHref = phoneRaw ? `tel:${phoneRaw.replace(/\s/g, '')}` : '';
    const personName = ((c.contact_person_name as string | undefined) ?? '')
        .toString()
        .trim();
    const personPhotoRaw =
        (c.contact_person_photo_url as string | null | undefined) ?? null;
    const personPhotoUrl =
        typeof personPhotoRaw === 'string' && personPhotoRaw.trim() !== ''
            ? personPhotoRaw.trim()
            : null;
    ...
    const externalImobiliareUrl = String(
        itemProps.external_imobiliare_url ??
            itemProps.externalImobiliareUrl ??
            '',
    ).trim();
    const externalOlxUrl = String(
        itemProps.external_olx_url ?? itemProps.externalOlxUrl ?? '',
    ).trim();

    // Removed: price drop alert + listing PDF download
    ...
    const slides = useMemo(() => {
        ...
        (gallery ?? []).forEach((g) =>
            list.push({ src: `/storage/${g.image_path}` }),
        );
        return list;
    }, [image_path, gallery]);
    ...
    const showNext = useCallback(() => {
        ...
        setLightboxIndex((prev) =>
            prev === null ? prev : (prev + 1) % slides.length,
        );
    ...
    const plainLead = short_description?.trim()
        ? stripHtml(short_description)
        : '';
    const leadText =
        plainLead.length > 280
            ? `${plainLead.slice(0, 280).trimEnd()}…`
            : plainLead;
    ...
    const pageDescription =
        ...
        (tPortfolio.section_body ??
            'Vezi detalii complete despre această proprietate publicată de Casa Imobby.');
    ...
    const galleryCountLabel = (
        tPortfolio.gallery_image_count ?? ':count images'
    ).replace(':count', String(photoCountForBadge));
            ...
            <div className="min-h-screen overflow-x-hidden bg-gradient-to-b from-background via-background to-neutral-50 text-foreground dark:from-neutral-950 dark:via-neutral-950 dark:to-neutral-900">
                    ...
                    <div className="grid gap-8 lg:grid-cols-12 lg:gap-10">
                        <div className="min-w-0 space-y-6 lg:col-span-8">
                            ...
                            {image_path ? (
                                <button
                                    type="button"
                                    className="group relative block w-full overflow-hidden rounded-xl ring-1 ring-border/60 focus-visible:ring-2 focus-visible:ring-brand focus-visible:outline-none"
                                ...
                                >
                                    <img
                                        ...
                                        fetchPriority="high"
                                        decoding="async"
                                        className="aspect-[4/3] max-h-[min(88vh,40rem)] w-full object-cover transition duration-300 group-hover:scale-[1.01] sm:aspect-[3/2] sm:max-h-[min(90vh,46rem)] lg:aspect-[16/10] lg:max-h-[min(92vh,52rem)]"
                                    />
                                    <span className="absolute right-3 bottom-3 rounded-full bg-black/55 px-3 py-1 text-xs font-medium text-white backdrop-blur-sm">
                                        {galleryCountLabel}
                                    </span>
                                </button>
                            ) : (
                            ...
                            {galleryList.length > 0 ? (
                                <section
                                    className="space-y-3"
                                    aria-labelledby="listing-gallery-more-heading"
                                >
                                    <h2
                                        id="listing-gallery-more-heading"
                                        className="text-base font-semibold sm:text-lg"
                                    >
                                        {tPortfolio.project_gallery ??
                                            'Gallery'}
                                    </h2>
                                    <div className="-mx-1 px-1 sm:-mx-2 sm:px-2">
                                        <div
                                            className="flex touch-pan-x snap-x snap-mandatory gap-3 overflow-x-auto overscroll-x-contain pt-1 pb-2 [scrollbar-width:thin] [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-track]:bg-transparent"
                                            aria-label={
                                                tPortfolio.project_gallery ??
                                                'Gallery thumbnails'
                                            }
                                        ...
                                        >
                                            {galleryList.map((img, index) => (
                                                <button
                                                    key={img.id}
                                                    type="button"
                                                    className="w-44 shrink-0 snap-start overflow-hidden rounded-lg ring-1 ring-border/60 transition hover:ring-brand/40 focus-visible:ring-2 focus-visible:ring-brand focus-visible:outline-none sm:w-52 lg:w-56"
                                                    onClick={() =>
                                                        setLightboxIndex(
                                                            image_path
                                                                ? index + 1
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            {personName || personPhotoUrl ? (
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    />
                                                    {personName ? (
                                                        <p className="min-w-0 text-sm leading-snug font-medium text-foreground">
                                                            {personName}
                                                        </p>
                                                    ) : null}
                                                </div>
                                        ...
                                        <div className="mt-8 flex flex-col gap-2">
                                            {phoneRaw ? (
                                                <Button
                                                    asChild
                                                    size="lg"
                                                    className="w-full justify-center gap-2 border-0 bg-gradient-to-r from-brand to-brand-accent text-white shadow-md transition-[filter,box-shadow] hover:shadow-lg hover:brightness-105 focus-visible:ring-white/40 dark:hover:brightness-110"
                                                >
                                                    <a href={telHref}>
                                                        <Phone
                            ...
                            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                                {similarItems.map((item) => (
                                    ...
                                    >
                                        ...
                                        <p className="mt-3 line-clamp-2 text-sm leading-snug font-semibold text-foreground group-hover:text-brand">
                                            {item.title}
                                        </p>
                                        {item.date ? (
                ...
                {hasLightbox && currentLightboxSrc && (
                    ...
                    >
                        <button
                            type="button"
                            className="absolute top-3 right-3 z-30 rounded-full bg-white/15 px-3 py-1.5 text-sm font-medium text-white backdrop-blur-sm hover:bg-white/25 sm:top-5 sm:right-5"
                            onClick={(e) => {
                                e.stopPropagation();
                                onCloseLightbox();
                            }}
                        ...
                        >
                            {slides.length > 1 ? (
                                <div className="absolute inset-0 z-0 flex min-h-[min(60vh,22rem)]">
                                    <button
                                        type="button"
                                        className="h-full min-h-[12rem] flex-1 cursor-w-resize bg-transparent focus-visible:ring-2 focus-visible:ring-white/40 focus-visible:outline-none sm:min-h-0"
                                        aria-label={
                                            tPortfolio.lightbox_prev ??
                                            'Previous image'
                                        }
                                        onClick={(e) => {
                                            e.stopPropagation();
                                    ...
                                    <button
                                        type="button"
                                        className="h-full min-h-[12rem] flex-1 cursor-e-resize bg-transparent focus-visible:ring-2 focus-visible:ring-white/40 focus-visible:outline-none sm:min-h-0"
                                        aria-label={
                                            tPortfolio.lightbox_next ??
                                            'Next image'
                                        }
                                        onClick={(e) => {
                                            e.stopPropagation();
                            ...
                            <img
                                ...
                                alt=""
                                className="pointer-events-none relative z-10 max-h-[min(90vh,52rem)] max-w-full rounded-lg object-contain shadow-2xl select-none"
                            />
                            {slides.length > 1 ? (
                                <>
                                    <button
                                        type="button"
                                        className="absolute top-1/2 left-1 z-20 flex size-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/55 text-white shadow-md backdrop-blur-sm transition hover:bg-black/70 focus-visible:ring-2 focus-visible:ring-white/60 focus-visible:outline-none sm:left-3 sm:size-12"
                                        aria-label={
                                            tPortfolio.lightbox_prev ??
                                            'Previous image'
                                        }
                                    ...
                                    <button
                                        type="button"
                                        className="absolute top-1/2 right-1 z-20 flex size-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/55 text-white shadow-md backdrop-blur-sm transition hover:bg-black/70 focus-visible:ring-2 focus-visible:ring-white/60 focus-visible:outline-none sm:right-3 sm:size-12"
                                        aria-label={
                                            tPortfolio.lightbox_next ??
                                            'Next image'
                                        }
